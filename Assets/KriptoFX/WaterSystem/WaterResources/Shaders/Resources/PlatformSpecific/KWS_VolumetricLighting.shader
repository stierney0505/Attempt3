			...
			half4 RayMarchDirLight(RaymarchData raymarchData, uint rayMarchSteps, bool isUnderwater)
			{
				...
				if (_DirectionalShadowIndex >= 0)
				{
					...
					for (uint j = 0; j < KWS_RayMarchSteps; ++j)
					{
						...
						context.shadowValue = lerp(1, (float)context.shadowValue, dirLight.volumetricShadowDimmer);
						...
						#endif
						float3 light = (float)context.shadowValue * scattering * lightColor.xyz * exposure;
						result.rgb += light;
